...
def login_email(request):
    ...
    return Response({"ok": True, "user": {"id": user.id, "email": decrypted_email}}, status=status.HTTP_200_OK)
...
def confirm_email(request, token):
    ...
    try:
        ...
        is_valid, error_msg = validate_and_clear_token(user, "first_name", "Confirmation token has expired.")
        if not is_valid:
...
def resend_confirmation(request):
    ...
    try:
        ...
        if not user.first_name or "|" not in user.first_name:
            return Response({"error": "Account is already confirmed"}, status=status.HTTP_400_BAD_REQUEST)
...
def reset_password(request, token):
    ...
    try:
        ...
        # Validate token expiration
        is_valid, error_msg = validate_and_clear_token(user, "last_name", "Password reset token has expired.")
        if not is_valid:
            return Response({"error": error_msg}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(create_success_response(
            ...
            {
                "id": user.id,
                "email": email_encryption.decrypt_email(user.email)
            }
        ), status=status.HTTP_200_OK)
        ...
        return Response({"error": f"Password reset failed: {e!s}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
