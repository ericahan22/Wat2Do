...
from apps.events.models import Events, EventDates, EventInterest, EventSubmission
...
class EventSourceValidator:
    ...
    def is_instagram_url(self, url):
        ...
        if not url:
            return False
        return bool(re.search(r'instagram\.com/p/|instagram\.com/reel/', url, re.IGNORECASE))
    ...
    async def check_instagram_post(self, session, url):
        ...
        try:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10), allow_redirects=True) as response:
                ...
                if b"Sorry, this page isn't available" in content or \
                   b"The link you followed may be broken" in content:
                    logger.warning(f"Instagram post page shows unavailable message: {url}")
                ...
                logger.warning(f"Instagram post returned status {response.status}: {url}")
        ...
        try:
            ...
            async with session.head(url, timeout=aiohttp.ClientTimeout(total=10), allow_redirects=True) as response:
                ...
                if response.status >= 400:
                    logger.warning(f"URL returned error status {response.status}: {url}")
                    return False, f"HTTP {response.status}"
    ...
    async def check_event_source(self, session, event):
        ...
        if self.is_instagram_url(event.source_url):
            is_valid, reason = await self.check_instagram_post(session, event.source_url)
        else:
    ...
    def delete_event(self, event, reason):
        ...
        try:
            with transaction.atomic():
                ...
                submissions_count = EventSubmission.objects.filter(created_event=event).count()
        ...
        async def validate_one(event):
            async with semaphore:
                try:
                    ...
                    self.stats['errors'] += 1
                    return None, f"Error: {str(e)}", event
